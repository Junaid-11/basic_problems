from tkinter import *
from demo.bookgui import back_db
window = Tk()
...
def view_command():
    list1.delete(0,END)
    for row in back_db.view():
        list1.insert(END, row)

def search_command():
    list1.delete(0,END)
    for row in back_db.search(title_text.get(),author_text.get(),year_text.get(), isbn_text.get()):
        list1.insert(END,row)

def add_book():
    back_db.insert(title_text.get(),author_text.get(),year_text.get(), isbn_text.get())
    list1.delete(0,END)
    list1.insert(END,(title_text.get(),author_text.get(),year_text.get(), isbn_text.get()))

def delete_book():
    back_db.delete(select_tup[0])

def update_book():
    back_db.update(select_tup[0], title_text.get(),author_text.get(),year_text.get(), isbn_text.get())
